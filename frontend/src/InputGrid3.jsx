		...
		try {
			...
			const res = await fetch('http://localhost:3000/ask', {
				...
				body: JSON.stringify({
					...
					Analyze the user's preferences and the PROVIDED DATA BELOW.
				
					CRITICAL: You MUST cite specific data from the context provided:
					- Quote exact rent prices (e.g., "Average rent: $1,914 according to CMHC")
					- Name specific schools from schoolsData (e.g., "Nearby schools include [school_name] - source: City of Vancouver Open Data")
					- Name specific dog parks from dogData by geo_local_area (e.g., "Off-leash dog parks in [geo_local_area] - source: City of Vancouver Open Data")
					- Name specific SkyTrain stations from skyTrainData (e.g., "Nearest station: [station] - source: City of Vancouver Open Data")
					
					If schoolsData, dogData, or skyTrainData are provided in the context, you MUST mention them by name in your response.
					
					FALLBACK RULE: If rentData, schoolsData, dogData, or skyTrainData are empty or missing for the user's area (e.g., locations outside Vancouver like 
					Burnaby, Richmond, Surrey), use your general knowledge to provide estimates. Clearly indicate when you are using general knowledge vs. provided data (e.g., "Based on general market data, average rent in Burnaby is approximately $X" vs. "Average rent: $1,914 according to CMHC").
					...
					"living_detail": "Detailed breakdown of 2-3 specific neighborhoods with rent prices from rentData. Include specific school names from schoolsData and dog park locations from dogData if provided.",
					"commute_detail": "Specific transit directions using station names from skyTrainData if provided.",
					"financialImpact": "Detailed savings projection using actual rent figures from rentData.",
